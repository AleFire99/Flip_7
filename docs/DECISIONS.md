...
**Addendum (issue #13, concern 2 -- exact investigation, scope-limited on purpose):** `flip7 modifier-effect` (`cmd_modifier_effect` in `src/flip7/cli.py`) replaces this ADR's Monte Carlo cell-voting with an exact, deterministic paired delta: for `unique_count` 0-6, with held numbers fixed to one of two representative sets ("low" = `{0, 1, ..., unique_count-1}`, "high" = `{12, 11, ..., 12-unique_count+1}`) and `remaining` = the full deck minus those held numbers (no sampling, no depletion noise), it computes `lookahead_ev`/`score_line` across `has_x2 ∈ {False, True}` × `plus_total ∈ {0, 5, 10}` (42 exact points per representative) and checks whether the hit/stay verdict ever flips from toggling a modifier alone.

- **Result: no flip, for either representative, at any unique_count.** Modifiers never changed the hit/stay verdict in this exact grid -- for "low", every cell recommends `hit` (0 through 6 unique cards); for "high", `unique_count=0-1` recommend `hit` and `unique_count=2-6` are an **exact tie** (`lookahead_ev` output equals `stay_value` to the decimal, at every tested `has_x2`/`plus_total` combination), which this report's strict `>` resolves as `stay` -- the same tie-breaking convention `evaluate_cell` already uses. This is real evidence for concern 2's case (a) (modifiers genuinely don't move this particular decision), not proof it holds everywhere: only 2 of the arbitrarily many possible held-number identities per count were tested, chosen specifically to bracket the low- and high-risk extremes.
- **This investigation independently reproduces concern 1 (identity vs. count) as a concrete, exact effect, not just a suspicion:** the "low" and "high" representatives disagree with each other, and with the shipped chart, everywhere except `unique_count=0`. "low" recommends `hit` at every count 0-6 (including 6 -- one card from Flip 7 -- because values 0-5 barely duplicate in the deck); "high" is an exact hit/stay tie from count 2 onward. The shipped chart's own "stay at 3+" rule sits *between* these two extremes, consistent with its Monte Carlo sampling (weighted by deck copies, so it leans toward higher-value, higher-risk hands) landing somewhere in between rather than at either pole. **Concern 1's fix direction (e.g. bucketing on `p_bust` instead of raw count) remains an open, deferred decision for a follow-up session** -- this addendum only supplies exact numbers for that session to start from, per the repo owner's Q&A: it does not redesign `basic_strategy.py`'s state axes.

## ADR-014: Turn-by-turn trace/replay viewer (issue #11)

**Context:** All existing verification is unit tests plus aggregate simulation stats (`reports/summary.txt`, `win_rates.png`). There was no way for a human to read one game's actions turn-by-turn and visually confirm the engine matches `docs/RULES_PHASE1.md`/`docs/RULES_PHASE2.md` -- who drew what, what happened, and for Freeze/Flip Three, who was targeted and by which mechanism.

**Decision:**

- `flip7.engine.TraceEvent` is a flat dataclass (`kind`, `round_no`, `seat`, `card`, `outcome`, `target`, `target_via`, plus `dealer`/`scores`/`totals`/`flip7_seat` for round-boundary events) appended to an optional `trace: list[TraceEvent] | None` parameter threaded through `play_round`/`play_game`/`_draw_and_resolve`/`_choose_action_target`. `trace=None` (the default, unchanged for every existing caller) records nothing, so this is a strictly additive, zero-cost-by-default change.
- `_choose_action_target` now returns `(target, via)` where `via` is `"policy"` or `"default"`, so a Freeze/Flip Three event can record which mechanism (a `TargetingPolicy` or the ADR-010 fallback) picked the target.
- Freeze/Flip Three/Second Chance draws still resolve exactly as before; they additionally log one `TraceEvent` each. A Flip Three's up-to-3 forced draws recurse through `_draw_and_resolve` tagged with the *same* `phase` ("deal" or "hit") as the card that triggered them, so they read as a contiguous sub-sequence in the trace rather than a separate, unlabeled phase.
- **Scope gap closed along the way:** `play_game` previously had no way to select the 94-card action-card deck (`full_deck_with_actions()`) -- it always built the plain 85-card deck via `play_round(deck=None)`. Since issue #11 explicitly requires tracing to work on both decks, `play_round` and `play_game` gained a `use_action_cards: bool = False` parameter selecting `full_deck(include_action_cards=...)` when `deck is None`.
- `flip7 replay --seed <n> [--policies ...] [--deck 85|94] [--target N] [--max-rounds N]` (`cmd_replay` in `src/flip7/cli.py`) runs one game via `play_game` with `trace=[]`, formats it with `_format_trace` (one line per event, grouped under `=== Round N (dealer: seat D) ===` headers), and writes+prints `reports/replay.txt`, matching every other `cmd_*`'s file+stdout convention.

**Consequences:** Any registered policy combination and either deck size can now be traced and read by a human as a complement to (not replacement for) the unit-test suite. The trace is a diagnostic/verification tool, not a gameplay feature -- it has no bearing on simulated win rates, and `trace=None` keeps every pre-existing call site (including `simulate_games`'s hot loop) exactly as fast as before.

## ADR-015: Per-strategy empirical diagnostics (issue #14)

**Context:** `flip7 analyze`/`compare`/`basic-strategy` report aggregate stats across a *lineup* of policies (win %, mean totals). There was no way to look at *one* policy's behavior in depth: does its real, observed hit/stay behavior actually match `flip7.basic_strategy`'s analytically-generated chart? Does bust risk actually escalate the way the design intends? What does its score distribution look like? (Distinct from issue #11's turn-by-turn single-game trace -- this is an aggregate, many-games strategy-validation tool.)

**Decision:**

- `play_round`'s hit loop now logs one `TraceEvent(kind="decide", ...)` per real `Policy.decide` call, carrying the *actual* game state it saw (`state_unique_count`, `state_has_x2`, `state_plus` -- the same axes as `flip7.basic_strategy.ALL_CELLS`) and the *actual* resulting action (`decision`; forced-stay-on-empty-pile is recorded as `"stay"`, not whatever the policy technically returned). This is purely additive logging alongside the existing `"hit"`-kind card-draw event for a stay (issue #11's replay format is untouched).
- `flip7.diagnostics.process_trace` walks a game's trace, building: an empirical decision chart (`Cell -> [hits, total]`, directly comparable to `BASIC_STRATEGY_CHART`), and three bust-rate breakdowns -- by `unique_count` at decision time, by round number within the game, and by the actual number value drawn -- by finding, for every `"decide"` event with `decision == "hit"`, the very next same-seat event (that seat's own draw outcome; forced Flip-Three draws on an *other* seat are correctly excluded, since they have no preceding `"decide"` call of their own).
- `flip7.diagnostics.run_diagnostics(policy, name, n_players=3, n_games=500, ...)` self-plays `policy` in every seat for `n_games` independent games (mirroring `simulate_games`'s loop), processing and discarding one game's trace at a time so memory stays bounded by a single game rather than growing with `n_games`, and pools every seat's final total as the policy's score distribution (self-play, so every seat is an equally valid sample).
- `flip7 diagnostics --policy NAME [--players N] [--games N] [--seed S]` (`cmd_diagnostics` in `src/flip7/cli.py`) writes `reports/diagnostics_<name>.txt` (score-distribution stats, the empirical-vs-shipped-chart table, and all three bust-rate tables) plus `..._score_hist.png` and `..._bust_rate.png` (3-panel: by `unique_count`/round/value).

**Consequences:** This is a real behavioral check, not just a performance number -- e.g. running it against `one_step_ev` shows it empirically hits at `unique_count=6` roughly 90% of the time, where the shipped `basic_strategy` chart's static rule says `stay`, consistent with ADR-013's own note that the chart is "materially more cautious" than `one_step_ev`/`lookahead_ev`'s early/late-game behavior -- and bust rate by drawn value climbs from 0% (values 0/1, one copy each in the deck) to ~17% (value 12, twelve copies), a real, empirical confirmation of `flip7.cards.NUMBER_COUNTS`'s design rather than just the analytical `p_bust` formula. Like ADR-014's trace, this only ever *reads* game state via `trace`; it has no effect on any existing simulation path.

## ADR-016: Basic-strategy chart re-keyed on P(bust) buckets (issue #13, concern 1)

**Context:** ADR-013's addendum left concern 1 (identity vs. count) as an open, deferred decision, explicitly naming "bucketing on `p_bust` instead of raw count" as the fix direction, backed by an exact finding: held numbers `{0,...,k-1}` ("low") recommend `hit` at every `unique_count` 0-6, while `{12,...,12-k+1}` ("high") are an exact hit/stay tie from `unique_count=2` onward -- the shipped chart's "stay at 3+" rule sits between both extremes and is wrong for both. This session ran a further exhaustive check: for every `unique_count` 0-6, every possible held-number identity (`itertools.combinations(range(13), unique_count)`, 4096 states total), computing exact `p_bust` (`flip7.probability.p_bust`, unchanged) and the true `lookahead_ev` hit/stay verdict. An idealized rule -- 2 hand-picked `p_bust` thresholds, one per `near_flip7 = (unique_count == 6)` state -- reproduces the true verdict on ~97.4% of all 4096 states, against ~82% for the best possible single global `p_bust` threshold with no `near_flip7` split; this motivated `PBUST_BUCKETS`' boundaries (10%/27%/40%). The true hit/stay boundary sits at `p_bust≈0.27` for `unique_count<6` and `p_bust≈0.40` at `unique_count==6` -- the Flip 7 +15 bonus shifts the safe threshold, independent of bust risk itself. The actual shipped chart (generated by Monte Carlo sampling over `lookahead_ev`, not the idealized rule above) measures at **94.0%** exact agreement against this same 4096-state ground truth, against **55.3%** for the old count-only rule ("hit iff `unique_count<=2`") -- both reproducible via `flip7 pbust-bucket-check`, added by this ADR (see below).

Bonus finding from the same sweep: `sum(held card values)` correlates with true `p_bust` at r=0.9977, because `NUMBER_COUNTS` sets `count(v)=v` for `v=2..12` -- a held value's own bust-risk contribution scales almost linearly with its pip value, and that sum is the same subtotal `score_line` already computes before `x2`/`plus`/the Flip 7 bonus. Rather than one flat threshold, the best single sum threshold *per unique_count* was fit against this same 4096-state ground truth (`flip7.basic_strategy.HELD_VALUE_SUM_STAY_THRESHOLDS`):

  | unique cards held | 0 | 1 | 2 | 3 | 4 | 5 | 6 (near Flip 7) |
  |---|---|---|---|---|---|---|---|
  | stay once sum reaches | never | never | 23 | 24 | 25 | 27 | 36 |

This reproduces the true verdict on **97.8%** of all 4096 states -- tighter than a flat 2-number rule, still just 5 numbers to memorize. At each threshold, the real P(bust) among every held-number combination with that exact sum is: 25.3% at sum=23 (`unique_count=2`), 25.6% at 24 (`unique_count=3`), 25.9-27.2% at 25 (`unique_count=4`), 27.5-28.7% at 27 (`unique_count=5`), 38.0-39.2% at 36 (`unique_count=6`) -- i.e. the tally's thresholds sit right at `PBUST_BUCKETS`' own `27%`/`40%` boundaries, confirming it's tracking the same underlying signal, not a coincidentally-correlated but different one. `flip7.basic_strategy.tally_recommend(unique_count, held_value_sum)` implements this table (documentation/printed cheat sheet only -- not used by `BasicStrategy.decide`), and `flip7 basic-strategy`'s `reports/basic_strategy.txt` prints it alongside the exact chart.

**Decision:**

- **New state-space axes** (`flip7.basic_strategy.Cell = tuple[str, bool, bool, str]`):
  - A coarse **P(bust) bucket** (`PBUST_BUCKETS`: `"<10%"`, `"10-27%"`, `"27-40%"`, `"40%+"`) -- this line's exact `p_bust(held_numbers, remaining)` against the live remaining deck. This replaces raw `unique_count` as the primary axis.
  - `near_flip7` (`unique_count == 6`) -- kept as a distinct boolean axis, not a reintroduction of raw count: it shifts the safe threshold itself (0.27 -> 0.40) for a genuinely separate reason (the Flip 7 bonus), not because card count carries its own signal.
  - `has_x2` and the existing 3-bucket plus-total label are kept in the mechanical grid (`ALL_CELLS`, `4 * 2 * 2 * 3 = 48` cells) for continuity -- `stay_value` still depends on them even where the verdict doesn't -- but are demoted: whether they still wash out into a small number of meaningfully-distinct rows is re-validated under the new primary axis (via `flip7 pbust-bucket-check`/`flip7 diagnostics`, see below), not assumed from ADR-013's addendum, which only tested this under the old raw-`unique_count` axis.
  - One cell, `("<10%", True, *, *)`, is structurally unreachable (the minimum possible `p_bust` at `unique_count=6` is ~12.7% -- 0-5 barely duplicate, but never zero) -- kept in `ALL_CELLS` for grid uniformity. `BasicStrategy.decide` never actually looks it up in real play, but it is still populated (see below), and reads `"hit"`: if that P(bust) *were* reachable at `unique_count=6`, the true verdict would be `hit` (it's deep in hit territory, which extends to `p_bust≈0.3973` there), so an initial version that read `"stay"` here was a bug, not just cosmetic.
- **Sampling pipeline** (`flip7.basic_strategy._sample_held_and_remaining_for_bucket`): rejection-samples held numbers (via the existing deck-copy-weighted `_sample_held_numbers`) and a depleted remaining deck (`_sample_remaining_deck`, unchanged) until the exact `p_bust` lands in the target bucket, falling back to a deterministic greedy search (add the deck's most-duplicated values first) if 300 random attempts miss -- the same rejection-then-exhaustive-fallback shape `_sample_plus_subset` already used for the plus axis. At `near_flip7`, the held count is fixed (can't grow incrementally), so the fallback delegates to `_closest_fixed_k_numbers`: for an unreachable bucket (only `"<10%"` at `unique_count=6` in practice), it tries both extremes -- the 6 least- and 6 most-duplicated values -- and keeps whichever lands closer to the target bucket, rather than always assuming "more duplication" is the right direction (which is only true when *growing* an incrementally-built hand, not when the count is already fixed at its cap).
- **Live decision** (`flip7.strategy.BasicStrategy.decide`): computes `p_bust(line.numbers, view.remaining)` **exactly**, not the cheap tally -- `BasicStrategy` already has perfect info per ADR-002 (`view.remaining` is free), so there is no reason to throw that away in favor of an approximation. The cheap `sum(held card values)` tally (97.8% agreement, see above) is documented here and printed in `reports/basic_strategy.txt` purely as a human-at-the-table appendix -- a deliberate, stated divergence between what the code computes and what a human without exact deck knowledge can compute in their head.
- **A genuine, newly-confirmed modifier effect** (not assumed from ADR-013's addendum, which only tested two fixed representatives at the old axis): the generated chart shows `has_x2`/plus DO shift the verdict at `near_flip7=True`, `"27-40%"` P(bust) specifically -- e.g. `has_x2=False, plus="0"` recommends `hit` there while `plus="1-5"`/`"6+"` recommend `stay`, and `has_x2=True` recommends `stay` across the board at that bucket. This sits exactly where the true hit/stay boundary (`p_bust≈0.3973`) is closest to a bucket's own edge (`"27-40%"` spans 0.27-0.40) -- a real, if narrow, case where a modifier tips an already-marginal cell, not a contradiction of ADR-013's addendum (which never tested this specific bucket/axis combination).
- **Diagnostics threading:** `flip7.engine.TraceEvent` gained `state_p_bust: float | None`, logged alongside the existing `state_unique_count`/`state_has_x2`/`state_plus` fields at the same `"decide"` event (all kept, purely additive). `flip7.diagnostics.DiagnosticsReport` gained `bust_by_pbust_bucket`, alongside (not replacing) `bust_by_unique_count`.
- **New exact validation tool**, `flip7 pbust-bucket-check` (`cmd_pbust_bucket_check`, mirrors `cmd_modifier_effect`'s deterministic style): exhaustively enumerates every held-number identity per `unique_count` and reports the new chart's exact agreement rate against ground truth alongside the old count-only rule's, plus the low/high-representative bucket-separation check that closes ADR-013's addendum gap directly.

**Consequences:** `BASIC_STRATEGY_CHART` is regenerated (48 entries) via `flip7 basic-strategy --seed 1 --out reports`; `reports/basic_strategy.txt`/`.png` and `flip7 diagnostics`'s per-cell table are restructured around the new axes. Cells straddling the true hit/stay boundary within a bucket (e.g. `"27-40%"` at `near_flip7=True`, where the true boundary sits at ~0.3973, close to the bucket's own edge) remain close to a coin flip in the same way ADR-013's own near-boundary count cells were -- coarse bucketing trades some precision for memorizability, same as the original design's premise. The regenerated chart's win-rate/EV gap against `lookahead_ev` (`reports/basic_strategy.txt`) is **-3.0 percentage points** (48.5% vs 51.5%, 200 games, seed=1) -- `reports/` is gitignored so no prior committed number exists to diff directly against, but this is a small fraction of ADR-013's own qualitative description of the old chart as "materially more cautious" than `lookahead_ev`'s early-game behavior, consistent with the ~39-point jump in exact agreement (55.3% -> 94.0%) `flip7 pbust-bucket-check` measures directly. `tests/test_basic_strategy.py`/`tests/test_strategy.py`/`tests/test_engine.py` were updated in lockstep, including direct regression tests encoding the fixed blind spot: a low-value hand (`{0..5}`, one card from Flip 7) now hits where the old count-only chart forced `stay` at 3+ cards, and a high-value hand (`{12,11,10,9}`, only 4 cards) now stays where the old chart forced `hit` below 3 cards; `tests/test_engine.py` drives this through a real round via the engine, not just an isolated `.decide()` call.
