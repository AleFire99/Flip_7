...
Bonus finding from the same sweep: `sum(held card values)` correlates with true `p_bust` at r=0.9977, because `NUMBER_COUNTS` sets `count(v)=v` for `v=2..12` -- a held value's own bust-risk contribution scales almost linearly with its pip value, and that sum is the same subtotal `score_line` already computes before `x2`/`plus`/the Flip 7 bonus. A human at the table can use this cheap running tally (roughly: stay once it reaches ~26, ~35 if one card from Flip 7) as a practical stand-in for `p_bust` without exact deck counting.
